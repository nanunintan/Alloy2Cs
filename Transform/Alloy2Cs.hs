module Transform.Alloy2Cs (alloy2cs) where
...
test f = do p <- parseFile f
            putStrLn $ pretty $ alloy2cs p

-- This needs to be updated to a partial transformation instead of ignoring unsupported features
alloy2cs :: Module -> Namespace
alloy2cs ns = let m = meta ns
              in Namespace  (name m) (classes m ++ enums m)
